...
public class BoardService {
    ...
    /** 시간 포맷팅 **/
    public List<String> getFormatTime(BoardPagingDto boardPagingList){
        List<String> formatTime = new ArrayList<>();

        for(BriefBoardInfo time : boardPagingList.getSimpleLectureDtoList()){
            String createdDate = time.getCreatedDate();
            DateTimeFormatter inputFormatter = DateTimeFormatter.ISO_LOCAL_DATE_TIME;
            DateTimeFormatter outputFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm");
            LocalDateTime date = LocalDateTime.parse(createdDate, inputFormatter);
            String formattedDate = date.format(outputFormatter);
            formatTime.add(formattedDate);
        }

        return formatTime;
    }

    public boolean compareAuth(String userRole, String boardRole){
        log.info("userRole, boardRole : {} {}", userRole, boardRole);
        if(boardRole == "MANAGER"){
