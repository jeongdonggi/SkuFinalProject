package com.SecurityBoardEx.BoardEx.jwt.filter;
...
class JwtAuthenticationProcessingFilterTest {
    ...
    @BeforeEach
    private void init(){
        userRepository.save(UserEntity.builder().username(USERNAME).password(delegatingPasswordEncoder.encode(PASSWORD))
                .nickname("Member1").role(Role.USER).build());
        clear();
    }
     ...
     */
    @Test
    public void 유효한RefreshToken이랑_유효한AccessToken_같이보냈을때_AccessToken_재발급_OK() throws Exception {
        //given
        Map accessAndRefreshToken = getAccessAndRefreshToken();
        String accessToken= (String) accessAndRefreshToken.get(accessHeader);
        String refreshToken= (String) accessAndRefreshToken.get(refreshHeader);
        ...
        assertThat(subject).isEqualTo(ACCESS_TOKEN_SUBJECT);
        assertThat(responseRefreshToken).isNotNull();//refreshToken 재발급
    }
     ...
     */
    @Test
    public void 유효한RefreshToken이랑_안유효한AccessToken_같이보냈을때_AccessToken_재발급_OK() throws Exception {
        //given
        Map accessAndRefreshToken = getAccessAndRefreshToken();
        String accessToken= (String) accessAndRefreshToken.get(accessHeader);
        String refreshToken= (String) accessAndRefreshToken.get(refreshHeader);
        ...
        assertThat(subject).isEqualTo(ACCESS_TOKEN_SUBJECT);
        assertThat(responseRefreshToken).isNotNull();//refreshToken은 재발급
    }
